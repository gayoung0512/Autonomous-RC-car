...
cap = cv2.VideoCapture('capstone_data/output_yolo.avi')
...
def regression(sl,image,right_detect,left_detect):#두 차선 교차하는 지점이 중심점으로부터 좌우 어느쪽에 있는지
    ...
    image = cv2.line(image, (right_ini_x,h), (right_fni_x,400), (0, 50, 50), 30, cv2.LINE_AA)
    image = cv2.line(image, (left_ini_x,h),(left_fni_x,400), (0, 255, 255), 20, cv2.LINE_AA)
    return output,image
